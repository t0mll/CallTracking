using CallTraking.NEventSocket.Common.FreeSWITCH.Conference;
...
namespace CallTraking.NEventSocket.Common.FreeSWITCH.Messages
{
    public class ConferenceEvent : EventMessage
    {
        ...
        {
            if (other.EventName != Events.EventNames.Custom && other.Headers[HeaderNames.EventSubclass] != CustomEvents.Conference.Maintainence)
            {
                throw new InvalidOperationException(
                    $"Expected event of type Custom with SubClass conference::maintainance, got {other.EventName} instead");
            }
        }
        ...
        public ConferenceAction Action
        {
            get
            {
                ...
                if (Headers.TryGetValue(HeaderNames.Conference.Action, out headerValue))
                {
                    ...
                    throw new NotSupportedException($"Unable to parse Conference Action '{headerValue}'.");
                }
